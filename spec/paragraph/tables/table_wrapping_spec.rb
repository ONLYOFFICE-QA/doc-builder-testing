...
describe 'Add tables with wrap and no wrap' do
  ...
  it 'Table with wrap' do
    docx = DocBuilderWrapper.new.build_doc_and_parse('asserts/js/paragraph/tables/table_with_wrap.js')
  ...
  it 'Table with no wrap' do
    docx = DocBuilderWrapper.new.build_doc_and_parse('asserts/js/paragraph/tables/table_with_nowrap.js')
    expect(docx.elements[1].rows.length).to eq(3)
