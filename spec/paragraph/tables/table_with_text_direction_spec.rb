...
describe 'Add tables with different text direction' do
  ...
  it 'Table with text direction: Left To Right' do
    docx = DocBuilderWrapper.new.build_doc_and_parse('asserts/js/paragraph/tables/table_with_textdirection_ltr.js')
  ...
  it 'Table with text direction: Top To Bottom' do
    docx = DocBuilderWrapper.new.build_doc_and_parse('asserts/js/paragraph/tables/table_with_textdirection_ttb.js')
    expect(docx.elements[1].rows.length).to eq(3)
    ...
    it 'Table with text direction: Right To Left' do
      docx = DocBuilderWrapper.new.build_doc_and_parse('asserts/js/paragraph/tables/table_with_textdirection_rtl.js')
      expect(docx.elements[1].rows.length).to eq(3)
    ...
    it 'Table with text direction: Bottom To Top' do
      docx = DocBuilderWrapper.new.build_doc_and_parse('asserts/js/paragraph/tables/table_with_textdirection_btt.js')
      expect(docx.elements[1].rows.length).to eq(3)
