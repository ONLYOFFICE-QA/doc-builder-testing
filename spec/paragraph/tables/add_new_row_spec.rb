...
describe 'Add new row in table' do
  ...
  it 'Add new row in position' do
    docx = DocBuilderWrapper.new.build_doc_and_parse('asserts/js/paragraph/tables/table_add_row_in_position.js')
  ...
  it 'Add new row without position' do
    docx = DocBuilderWrapper.new.build_doc_and_parse('asserts/js/paragraph/tables/table_add_row_without_position.js')
    expect(docx.elements[1].rows.length).to eq(4)
