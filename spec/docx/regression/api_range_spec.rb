...
describe 'ApiRange regression tests' do
  it 'ApiRange | 71120 IntersectWith method' do
    skip('https://bugzilla.onlyoffice.com/show_bug.cgi?id=71590')
    ...
    expect(docx.elements[2].nonempty_runs[0].text).to eq("Another text\nStill text\n")
    expect(docx.elements[2].nonempty_runs[2].text).to eq("One more text\n")
