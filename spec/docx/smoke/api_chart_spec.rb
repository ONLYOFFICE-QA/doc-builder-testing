...
describe 'ApiChart section tests' do
  it 'ApiChart | GetClassType method' do
    ...
    expect(docx.elements[1].character_style_array.first.text).to eq('Class Type = chart')
  end
  ...
  it 'ApiChart | SetLegendPos method' do
    ...
    expect(docx.elements.first.character_style_array[1].drawings.first.graphic.data.legend.position).to eq(:bottom)
  end
  ...
  it 'ApiChart | SetShowDataLabels method' do
    ...
    expect(docx.elements.first.character_style_array[1].drawings.first.graphic.data.axises.first.title.elements.first.runs.first.text).to eq('Year')
    expect(docx.elements.first.character_style_array[1].drawings.first.graphic.data.axises.first.title.elements.first.runs.first.properties.font_size).to eq(11)
  end

  it 'ApiChart | SetTitle method' do
    docx = DocBuilderWrapper.new.build_doc_and_parse('asserts/js/docx/smoke/ApiChart/settitle.js')
    expect(docx.elements.first.character_style_array[1].drawings.first.graphic.data.title.elements.first.runs.first.text).to eq('Financial Overview')
    expect(docx.elements.first.character_style_array[1].drawings.first.graphic.data.title.elements.first.runs.first.properties.font_size).to eq(13.0)
  end
  ...
  it 'ApiChart | SetVerAxisTitle method' do
    ...
    expect(docx.elements.first.character_style_array[1].drawings.first.graphic.data.axises[1].title.elements.first.runs.first.text).to eq('USD In Hundred Thousands')
    expect(docx.elements.first.character_style_array[1].drawings.first.graphic.data.axises[1].title.elements.first.runs.first.properties.font_size).to eq(10)
  end
end
