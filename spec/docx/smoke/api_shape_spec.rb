...
describe 'ApiShape section tests' do
  it 'ApiShape | GetClassType method' do
    docx = builder.build_and_parse('js/docx/smoke/api_shape/get_class_type.js')
    expect(docx.elements[1].nonempty_runs.first.text).to eq('Class Type = shape')
  end

  it 'ApiShape | GetDocContent method' do
    docx = builder.build_and_parse('js/docx/smoke/api_shape/get_doc_content.js')
  ...
  it 'ApiShape | SetVerticalTextAlign method' do
    docx = builder.build_and_parse('js/docx/smoke/api_shape/set_vertical_text_align.js')
