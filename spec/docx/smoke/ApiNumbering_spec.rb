...
describe 'ApiNumbering section tests' do
  it 'ApiNumbering | GetClassType method' do
    ...
    expect(docx.elements.last.character_style_array.first.text).to eq('Class Type = numbering')
  end
  ...
  it 'ApiNumbering | GetLevel method' do
    ...
    docx.elements[1..docx.elements.size].each_with_index do |current_element, index|
      expect(current_element.character_style_array.first.text).to eq("Default bullet lvl #{index + 1}")
      expect(current_element.numbering.ilvl).to eq(index.to_s)
    end
  end
end
