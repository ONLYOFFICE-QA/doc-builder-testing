...
describe 'All tests numbering object' do
  it 'Cross-reference to numbering paragraphs' do
    ...
    expect(docx.elements[3].runs[3].instruction_text.value).to eq(' PAGEREF _Ref1 \\h')
    expect(docx.elements[4].nonempty_runs[1].text).to eq('1')
    expect(docx.elements[4].runs[3].instruction_text.value).to eq(' REF _Ref1 \\r  \\h')
    expect(docx.elements[5].nonempty_runs[1].text).to eq('1.2')
    expect(docx.elements[5].runs[3].instruction_text.value).to eq(' REF _Ref2 \\n  \\h')
    expect(docx.elements[6].nonempty_runs[1].text).to eq('1.*1.2')
    expect(docx.elements[6].runs[3].instruction_text.value).to eq(' REF _Ref2 \\w  \\h \\d *')
    expect(docx.elements[7].nonempty_runs[1].text).to eq('Numbered paragraph lvl 1')
    expect(docx.elements[7].runs[3].instruction_text.value).to eq(' REF _Ref1  \\h')
    expect(docx.elements[8].nonempty_runs[1].text).to eq('above')
    expect(docx.elements[8].runs[3].instruction_text.value).to eq(' REF _Ref1 \p  \h')
  end
  ...
  it 'Cross-reference to heading' do
    ...
    expect(docx.elements[1].runs[3].instruction_text.value).to eq(' REF _Ref1  \\h')
    # TODO: bug 'https://bugzilla.onlyoffice.com/show_bug.cgi?id=58290'
    # expect(docx.elements[2].nonempty_runs[1].text).to eq('1')
    expect(docx.elements[2].runs[3].instruction_text.value).to eq(' PAGEREF _Ref1 \\h')
    expect(docx.elements[3].nonempty_runs[1].text).to eq('0')
    expect(docx.elements[3].runs[3].instruction_text.value).to eq(' REF _Ref1 \\r  \\h \\p')
    expect(docx.elements[4].nonempty_runs[1].text).to eq('above')
    expect(docx.elements[4].runs[3].instruction_text.value).to eq(' REF _Ref1 \\p  \\h')
  end
  ...
  it 'Cross-reference to bookmarks' do
    ...
    expect(docx.elements[4].runs[2].instruction_text.value).to eq(' REF Bookmark \\p  \\h')
  end
  ...
  it 'Cross-reference to footnote' do
    ...
    expect(docx.elements[1].runs[3].instruction_text.value).to eq(' NOTEREF _Ref1 \\h')
    # TODO: bug 'https://bugzilla.onlyoffice.com/show_bug.cgi?id=58290'
    # expect(docx.elements[2].nonempty_runs[1].text).to eq('1')
    expect(docx.elements[2].runs[3].instruction_text.value).to eq(' PAGEREF _Ref2 \\h')
    expect(docx.elements[3].nonempty_runs[1].text).to eq('above')
    expect(docx.elements[3].runs[3].instruction_text.value).to eq(' NOTEREF _Ref1 \\p  \\h')
    expect(docx.elements[4].nonempty_runs[1].text).to eq('1')
    expect(docx.elements[4].nonempty_runs[1].run_properties.vertical_align.to_s).to eq('superscript')
    expect(docx.elements[4].runs[3].instruction_text.value).to eq(' NOTEREF _Ref1 \\f  \\h')
  end
