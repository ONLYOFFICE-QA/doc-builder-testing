...
describe 'Add tables with wrap and no wrap' do
  it 'Table with wrap' do
    docx = builder.build_and_parse('asserts/js/docx/paragraph/tables/table_with_wrap.js')
  ...
  it 'Table with no wrap' do
    docx = builder.build_and_parse('asserts/js/docx/paragraph/tables/table_with_nowrap.js')
    expect(docx.elements[1].rows.length).to eq(3)
