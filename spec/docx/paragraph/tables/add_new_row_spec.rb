...
describe 'Add new row in table' do
  it 'Add new row in position' do
    docx = builder.build_and_parse('js/docx/paragraph/tables/table_add_row_in_position.js')
  ...
  it 'Add new row without position' do
    docx = builder.build_and_parse('js/docx/paragraph/tables/table_add_row_without_position.js')
    expect(docx.elements[1].rows.length).to eq(4)
