...
describe 'ApiRange section tests' do
  ...
  it 'ApiRange | SetWrapText method' do
    xlsx = builder.build_doc_and_parse('asserts/js/xlsx/smoke/api_range/set_wrap_text.js')
    expect(xlsx.worksheets.first.rows[1].cells[0].style.alignment.wrap_text).to be_truthy
    expect(xlsx.worksheets.first.rows[1].cells[2].style.alignment.wrap_text).to be_falsey
  end
  ...
  it 'ApiRange | GetRowHeight method' do
    skip if builder.semver < Semantic::Version.new('5.2.0')
    xlsx = builder.build_doc_and_parse('asserts/js/xlsx/smoke/api_range/get_row_height.js')
  ...
  it 'ApiRange | RowHeight getter' do
    skip if builder.semver < Semantic::Version.new('5.2.0')
    xlsx = builder.build_doc_and_parse('asserts/js/xlsx/smoke/api_range/row_height_getter.js')
  ...
  it 'ApiRange | GetWrapText method' do
    skip if builder.semver < Semantic::Version.new('5.2.0')
    xlsx = builder.build_doc_and_parse('asserts/js/xlsx/smoke/api_range/get_wrap_text.js')
    expect(xlsx.worksheets.first.rows[1].cells.first.text).to eq('Text')
  end

  it 'ApiRange | SetOffset method' do
    skip if builder.semver < Semantic::Version.new('5.2.0')
    xlsx = builder.build_doc_and_parse('asserts/js/xlsx/smoke/api_range/set_offset.js')
    expect(xlsx.worksheets.first.rows[3].cells[1].text).to eq('Text')
  end

  it 'ApiRange | GetAddress method' do
    skip if builder.semver < Semantic::Version.new('5.2.0')
    xlsx = builder.build_doc_and_parse('asserts/js/xlsx/smoke/api_range/get_address.js')
    expect(xlsx.worksheets.first.rows[3].cells[1].text).to eq('Text')
  end
end
