...
describe 'ApiDrawing section tests' do
  it 'ApiDrawing | GetClassType method' do
    xlsx = builder.build_and_parse('asserts/js/xlsx/smoke/api_drawing/get_class_type.js')
    expect(xlsx.worksheets.first.rows.first.cells[1].text).to eq('shape')
  end

  it 'ApiDrawing | SetPosition method' do
    xlsx = builder.build_and_parse('asserts/js/xlsx/smoke/api_drawing/set_position.js')
  ...
  it 'ApiDrawing | SetSize method' do
    xlsx = builder.build_and_parse('asserts/js/xlsx/smoke/api_drawing/set_size.js')
